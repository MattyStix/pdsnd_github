...
def get_filters():
    ...
    while True:
        city = input(
            "\nPlease select a city to filter the data. You may choose 'Chicago', 'Washington' or 'New York City'.\n").lower()
        if city not in CITY_DATA:
            print(
                "\nUnfortunately data from that city is unavailable. Please, try again.\n")
            continue
        else:
            break
    ...
    while True:
        day = input(
            "\nPlease select a day of the week, Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday are available. The data from all days can be selected by choosing 'all'.\n").lower()
